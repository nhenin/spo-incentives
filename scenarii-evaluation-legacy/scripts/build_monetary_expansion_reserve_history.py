...
def main() -> None:
    ...
    fig_path = root / "scenarii-evaluation" / "figures" / "monetary_expansion_reserve_history_mainnet.png"
    notes_path = root / "scenarii-evaluation" / "outputs" / "monetary_expansion_reserve_history_mainnet.md"
    csv_path = root / "scenarii-evaluation" / "outputs" / "monetary_expansion_reserve_history_mainnet.csv"
    ...
    ax1.plot(epochs, to_billion(reserve), color="#1f77b4", linewidth=2.0, label=r"Reserve stock $(T_{\infty}-T)$")
    ax1.set_ylabel("Billion ADA")
    ax1.set_title("Cardano Mainnet Reserve Stock and Reserve-Sourced Monetary Expansion Since Shelley")
    ax1.legend(loc="upper right")
    ax1.text(
        0.01,
        0.02,
        f"Complete reserve window: epochs {rows[first_complete_idx].epoch_no}..{rows[last_complete_idx].epoch_no}\n"
        f"Reserve fell from {reserve_start/1e9:.3f}B to {reserve_end/1e9:.3f}B ADA "
        f"({reserve_pct_change:.1f}% vs start)",
    ...
    ax2.plot(epochs, to_million(nominal), color="#d62728", linewidth=1.8, label=r"Nominal $\rho \cdot Reserve$")
    ax2.plot(
        epochs,
    ...
    ax3.annotate(
        f"low complete\n{rows[low_pool_idx].epoch_no} {format_date(rows[low_pool_idx].start_time_utc)}\n{pool_side[low_pool_idx]/1e6:.2f}M",
    ...
    ax3.annotate(
        f"high complete\n{rows[high_pool_idx].epoch_no} {format_date(rows[high_pool_idx].start_time_utc)}\n{pool_side[high_pool_idx]/1e6:.2f}M",
